...
class StatisticsEnabledWriteHandler extends AbstractWriteAttributeHandler<Void> {
    ...
    void updateToRuntime(final OperationContext context, final ModelNode model) throws OperationFailedException {
        final ModelNode statisticsEnabledModel = STATISTICS_ENABLED.resolveModelAttribute(context, model);
        final boolean statisticsEnabled = statisticsEnabledModel.isDefined() && statisticsEnabledModel.asBoolean();
